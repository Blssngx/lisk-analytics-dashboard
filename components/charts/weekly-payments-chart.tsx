...
export function WeeklyPaymentsChart({
	data,
	symbol,
}: Readonly<{ data?: WeeklyPayments[]; symbol: string }>) {
	const [timeRange, setTimeRange] = React.useState("all");
	const [activeChart, setActiveChart] = React.useState<
		"combined" | "amounts" | "payments" | "average"
	>("combined");
	...
	const items = React.useMemo(() => {
		...
		const weeklyData = new Map<
			string,
			{
				...
				count: number;
				total: number;
				transactions: Array<{ count: number; total: number }>;
			}
		>();
	...
	const maxDate = React.useMemo(() => {
		const maxTimestamp = items.reduce((acc, p) => Math.max(acc, p.date.getTime()), 0);
	...
	const legendStats = React.useMemo(() => {
		const payments = Number(
			filteredData.reduce((acc, cur) => cur.payments + acc, 0).toFixed(2),
		);
		const totalAmount = Number(
			filteredData.reduce((acc, cur) => cur.amounts + acc, 0).toFixed(2),
		);
			...
			case "amounts":
				return (
					<BarChart {...commonProps}>
								...
								<ChartTooltipContent
									...
									formatter={(value, name, item) => {
										if (name === "amounts") {
											...
											return [
												`${Number(value || 0).toFixed(2)} ${symbol}`,
												`Total Amount | ${payments} payments | Avg: ${avg.toFixed(
													2,
												)} ${symbol}`,
											];
										}
						...
						/>
						<Bar dataKey="amounts" fill="var(--color-amounts)" radius={[4, 4, 0, 0]} />
					</BarChart>
				);
									...
									formatter={(value, name, item) => {
										if (name === "payments") {
											...
											return [
												...
												`Total: ${total.toFixed(
													2,
												)} ${symbol} | Avg: ${avg.toFixed(2)} ${symbol}`,
											];
										}
										return `${Number(value || 0).toLocaleString()}`;
									}}
									...
									formatter={(value, name, item) => {
										if (name === "avg") {
											...
											return [
												...
												`Average Payment | ${payments} payments | Total: ${total.toFixed(
													2,
												)} ${symbol}`,
											];
										}
