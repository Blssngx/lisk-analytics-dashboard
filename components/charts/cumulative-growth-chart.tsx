...
export function ChartAreaInteractive({
	...
	const maxDate = React.useMemo(() => {
		const maxTime = points.reduce((acc, p) => Math.max(acc, p.date.getTime()), 0);
		return new Date(maxTime);
	}, [points]);

	// Build recharts data with styled keys
	const rechartsData = React.useMemo(() => {
		// sort a copy of the points array so we don't mutate the original
		const sorted = points.toSorted((a, b) => a.date.getTime() - b.date.getTime());
		return sorted.map((p) => ({
				...
				<div className="grid grid-cols-3 gap-3">
					...
					<button
						...
						<div className="flex items-center gap-2 text-xs text-muted-foreground">
							...
							/>
							<span>
								{chartConfig.mobile.label} ({symbol})
							</span>
						</div>
						<div className="text-foreground text-xl font-bold">
							{legendStats.mobile.toLocaleString(undefined, {
