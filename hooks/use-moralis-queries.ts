...
const requestDuplicator = new RequestDuplicator();
...
const fetchMoralisTransactions = async (
...
const fetchMoralisTransfers = async (
	contractAddress: string,
	limit: number = 100,
...
const fetchCumulativeGrowth = async (contractAddress: string): Promise<any> => {
	...
	if (!response.ok) {
		...
		throw new Error(
			errorData.error || `Failed to fetch cumulative growth data (${response.status})`,
		);
	}
...
const fetchUniqueWallets = async (contractAddress: string): Promise<any> => {
	...
	if (!response.ok) {
		const errorData = await response.json().catch(() => ({}));
		throw new Error(
			errorData.error || `Failed to fetch unique wallets data (${response.status})`,
		);
	}
...
const fetchWeeklyPayments = async (contractAddress: string, methodId: string): Promise<any> => {
	...
	if (!response.ok) {
		const errorData = await response.json().catch(() => ({}));
		throw new Error(
			errorData.error || `Failed to fetch weekly payments data (${response.status})`,
		);
	}
...
const fetchTokenHolders = async (contractAddress: string): Promise<any> => {
	...
	if (!response.ok) {
		const errorData = await response.json().catch(() => ({}));
		throw new Error(
			errorData.error || `Failed to fetch token holders data (${response.status})`,
		);
	}
...
export const useMoralisTokenData = (contractAddress: string) => {
	return useQuery({
		...
		queryFn: () =>
			requestDuplicator.deduplicate(`moralis:token:data:${contractAddress}`, () =>
				fetchMoralisTokenData(contractAddress),
			),
...
export const useMoralisTransactions = (contractAddress: string, limit: number = 100) => {
	return useQuery({
		...
		queryFn: () =>
			requestDuplicator.deduplicate(`moralis:transactions:${contractAddress}:${limit}`, () =>
				fetchMoralisTransactions(contractAddress, limit),
			),
		enabled: !!contractAddress,
...
export const useMoralisTransfers = (contractAddress: string, limit: number = 100) => {
	return useQuery({
		...
		queryFn: () =>
			requestDuplicator.deduplicate(`moralis:transfers:${contractAddress}:${limit}`, () =>
				fetchMoralisTransfers(contractAddress, limit),
			),
		enabled: !!contractAddress,
		staleTime: 2 * 60 * 1000, // 2 minutes
		gcTime: 5 * 60 * 1000, // 5 minutes
...
export const useCumulativeGrowth = (contractAddress: string) => {
	return useQuery({
		...
		queryFn: () =>
			requestDuplicator.deduplicate(`moralis:cumulative-growth:${contractAddress}`, () =>
				fetchCumulativeGrowth(contractAddress),
			),
		enabled: !!contractAddress,
...
export const useUniqueWallets = (contractAddress: string) => {
	return useQuery({
		...
		queryFn: () =>
			requestDuplicator.deduplicate(`moralis:unique-wallets:${contractAddress}`, () =>
				fetchUniqueWallets(contractAddress),
			),
		enabled: !!contractAddress,
		staleTime: 0, // Always fetch fresh data
		gcTime: 5 * 60 * 1000, // 5 minutes
	...
	return useQuery({
		...
		queryFn: () =>
			requestDuplicator.deduplicate(
				`moralis:weekly-payments:${contractAddress}:${methodId}`,
				() => fetchWeeklyPayments(contractAddress, methodId),
			),
...
export const useMoralisTokenHolders = (contractAddress: string) => {
	return useQuery({
		...
		queryFn: () =>
			requestDuplicator.deduplicate(`moralis:token-holders:${contractAddress}`, () =>
				fetchTokenHolders(contractAddress),
			),
		enabled: !!contractAddress,
		staleTime: 0, // Always fetch fresh data
		gcTime: 5 * 60 * 1000, // 5 minutes
